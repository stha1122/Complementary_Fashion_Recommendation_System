...
def train_compatibility_model(starting_epoch=0, num_epochs=2, batch_size=32):
    ...
    criterion = torch.jit.script(torch.nn.TripletMarginLoss(margin=cfg.MARGIN)).to(device)
    training_losses = []
    validation_losses = []
    avg_training_losses = []
    ...
    if os.path.exists(
        ...
        checkpoint = torch.load(
            f"{cfg.TRAINED_MODEL_DIR}/trained_compatibility_model_epoch{starting_epoch - 1}.pth",map_location=device
        )
    ...
    for e in tqdm(range(num_epochs), desc="Epochs"):
                ...
                # get validation loss
                    ...
                    with torch.no_grad():
                        try:
                            ...
                            iterator = iter(validation_dataloader)
                            anchor_val, positive_val, negative_val = next(iterator)
        ...
        # Handle the case where the file is not found (image download failed)
            print(f"Skipping image - File not found: {e}")
            continue
